...
export async function generateDailyReadAudio(dailyReadId: string) {
  ...
  if (read.audio_url) {
    return { audioUrl: read.audio_url, generated: false, reason: "already_exists" };
  }
  ...
  try {
    ...
    return { audioUrl, generated: true, reason: "generated" };
  } catch (err) {
    const message = err instanceof Error ? err.message : "tts_failed";

    if (message.includes("quota") || message.includes("429")) {
      return { audioUrl: null, generated: false, reason: "quota_exceeded" };
    }

    return { audioUrl: null, generated: false, reason: "tts_failed" };
  }
}
